...
inputs:
  ...
  - id: gatk_jar
    type: File
    'sbg:x': 772.9586791992188
    'sbg:y': 202.2533721923828
  - id: outputfile_HaplotypeCaller
    type: string
    'sbg:exposed': true
  - id: targetsIntervalList
    type: File?
    'sbg:x': 830.3356323242188
    'sbg:y': -158.672119140625
outputs:
  - id: markDups_output_index
    outputSource:
      - picard__mark_duplicates/markDups_output_index
    type: File
    'sbg:x': 612.6316528320312
    'sbg:y': -169.16006469726562
  - id: output_HaplotypeCaller
    outputSource:
      - _g_a_t_k__haplotype_caller/output_HaplotypeCaller
    type: File
    'sbg:x': 1157.6480712890625
    'sbg:y': 17.00774383544922
steps:
  - id: bwa_mem
    in:
      - id: output_filename
  ...
  - id: _g_a_t_k__haplotype_caller
    in:
      ...
      - id: outputfile_HaplotypeCaller
        source: outputfile_HaplotypeCaller
      - id: reference
        source: reference
      - id: targetsIntervalList
        source: targetsIntervalList
    out:
      - id: output_HaplotypeCaller
